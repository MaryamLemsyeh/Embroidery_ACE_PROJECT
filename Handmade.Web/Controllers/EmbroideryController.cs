...
namespace Handmade.Web.Controllers
{
	public class EmbroideryController : Controller
	{
		...
		{
			var response = await _embroideryService.DeleteEmbroideriesAsync<ResponseDto>(model.EmbroideryId);
		...
		{
			var response = await _embroideryService.GetEmbroideryByIdAsync<ResponseDto>(model.EmbroideryId);
			if (response.IsSuccess)
			{
				return RedirectToAction(nameof(EmbroideryIndex));
			}
